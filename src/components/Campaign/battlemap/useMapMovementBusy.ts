...
import {isTokenMovePending,subscribeTokenMoves,tokenMoveRevision} from './pendingTokenMoves';
...
export function isMapMovementBusy(sceneId=useBattleMapStore.getState().currentSceneId):boolean {
  ...
  return Object.keys(state.tokens).some(id=>state.dragging===id || !!state.remoteDragLocks[id] || isTokenMovePending(id));
}
