...
export function MapNavigation({ viewport, canvas, selectedIds, gridSizePx, editingToolActive, onSelectMode }: {
  ...
  useEffect(() => {
    ...
    const down = (event: PointerEvent) => {
      if (event.button !== 0 || (!pan && !space) || drag) return;
    ...
    const blur = () => { space=false; if(drag && canvas.hasPointerCapture(drag.id)) canvas.releasePointerCapture(drag.id); drag=null; cursor(); };
