...
export default function NPCManager({ campaignId, isOwner }: NPCManagerProps) {
  ...
  if (loading) return <div className="loading-text" style={{ padding: 'var(--space-4)' }}>Loading NPCs…</div>;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
      ...
      <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        />
        <select value={filterRole} onChange={e => setFilterRole(e.target.value)} style={{ fontSize: 'var(--text-sm)', width: 'auto' }}>
          <option value="">All Roles</option>
          {ROLES.map(r => <option key={r} value={r}>{ROLE_ICONS[r]} {r.charAt(0).toUpperCase() + r.slice(1)}</option>)}
        </select>
        <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', color: 'var(--text-muted)', textTransform: 'none', letterSpacing: 0, marginBottom: 0 }}>
            ...
            {group.map(npc => {
              ...
              return (
                <div key={npc.id} style={{
                  border: `1px solid ${npc.is_alive ? roleColor + '30' : 'var(--border-subtle)'}`,
                  ...
                  {/* NPC row */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', padding: 'var(--space-3) var(--space-4)', cursor: 'pointer' }}
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
                        <span style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: 'var(--text-sm)', color: npc.is_alive ? 'var(--text-primary)' : 'var(--text-muted)' }}>
                          {npc.name}
                          {!npc.is_alive && ' ☠️'}
                        </span>
                        {npc.race && <span style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>{npc.race}</span>}
                        {npc.faction && (
                          <span style={{ fontFamily: 'var(--font-body)', fontSize: 9, fontWeight: 600, color: 'var(--color-arcane-bright)', background: 'rgba(91,63,168,0.12)', padding: '1px 6px', borderRadius: 999, border: '1px solid rgba(91,63,168,0.25)' }}>
                            {npc.faction}
                          </span>
                        )}
                      </div>
                    ...
                    {/* Relationship badge */}
                    <span style={{ fontFamily: 'var(--font-body)', fontSize: 9, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: roleColor, background: `${roleColor}15`, border: `1px solid ${roleColor}40`, padding: '2px 7px', borderRadius: 999, flexShrink: 0 }}>
                      {npc.relationship}
                    </span>
                  </div>
                  ...
                  {/* Expanded details */}
                  {isExpanded && (
                    <div className="animate-fade-in" style={{ padding: 'var(--space-3) var(--space-4)', borderTop: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                      {npc.description && (
                        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)', lineHeight: 1.6, margin: 0 }}>{npc.description}</p>
                      )}
                      {npc.notes && (
                        <div>
                          <div style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 4 }}>DM Notes</div>
                          <p style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)', lineHeight: 1.6, margin: 0, fontStyle: 'italic' }}>{npc.notes}</p>
                        </div>
                      )}
                      {isOwner && (
                        <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
                          ...
                          <button
                            ...
                            style={{ color: npc.is_alive ? 'var(--color-crimson-bright)' : 'var(--hp-full)' }}
                          >
                            {npc.is_alive ? '☠️ Mark Dead' : '💚 Revive'}
                          </button>
                          <button className="btn-ghost btn-sm" onClick={() => deleteNPC(npc.id)} style={{ color: 'var(--color-crimson-bright)', marginLeft: 'auto' }}>
                            Delete
                          </button>
                        </div>
                      )}
      ...
      {editing && (
        <div className="modal-overlay" onClick={() => setEditing(null)}>
          <div className="modal" style={{ maxWidth: 520 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ marginBottom: 'var(--space-4)' }}>{(editing as NPC).id ? 'Edit' : 'New'} NPC</h3>
