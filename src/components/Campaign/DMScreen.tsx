  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', height: '100%' }}>
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 'var(--text-sm)', color: '#fca5a5' }}>
      ...
      <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid var(--border-subtle)', paddingBottom: 0 }}>
        {SECTIONS.map(s => (
          <button key={s.id} onClick={() => setActiveSection(s.id)} style={{
            fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 'var(--text-xs)', letterSpacing: '0.06em',
            ...
            color: activeSection === s.id ? 'var(--text-gold)' : 'var(--text-muted)',
          ...
          {!combatActive || combatants.length === 0 ? (
            ...
            return (
              <div key={c.id} style={{
                ...
                padding: 'var(--space-3) var(--space-4)',
                borderRadius: 'var(--radius-md)',
                border: isActive ? '1px solid var(--color-gold)' : '1px solid var(--border-subtle)',
                background: isActive ? 'rgba(201,146,42,0.08)' : 'var(--bg-sunken)',
                transition: 'all var(--transition-fast)',
              }}>
                <div style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 'var(--text-sm)', color: 'var(--text-gold)', minWidth: 24 }}>
                  {c.initiative}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                    {isActive && <span style={{ fontSize: 10, color: 'var(--color-gold-bright)' }}>▶</span>}
                    <span style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 'var(--text-sm)', color: isActive ? 'var(--text-gold)' : 'var(--text-primary)' }}>
                      {c.name}
                    </span>
                    {c.is_monster && <span style={{ fontFamily: 'var(--font-heading)', fontSize: 9, color: 'var(--color-crimson-bright)', background: 'rgba(155,28,28,0.15)', padding: '1px 5px', borderRadius: 3 }}>Monster</span>}
                  </div>
                  {c.is_monster && (
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <span style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 'var(--text-sm)', color: hpColor }}>
                    {c.current_hp}/{c.max_hp}
                  </span>
                  <div style={{ fontFamily: 'var(--font-heading)', fontSize: 9, color: 'var(--text-muted)' }}>AC {c.ac}</div>
                </div>
              </div>
            );
          })}
      ...
      {activeSection === 'party' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
          {party.map(m => {
            ...
            const hpColor = hpPct > 0.5 ? 'var(--hp-full)' : hpPct > 0.25 ? 'var(--hp-mid)' : hpPct > 0 ? 'var(--hp-low)' : 'var(--hp-dead)';
            return (
              <div key={m.id} style={{ display: 'flex', gap: 'var(--space-3)', alignItems: 'center', padding: 'var(--space-3) var(--space-4)', background: 'var(--bg-sunken)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)' }}>
                <div style={{ width: 36, height: 36, borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-subtle)', overflow: 'hidden', flexShrink: 0, background: 'var(--bg-raised)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginBottom: 2 }}>
                    <span style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 'var(--text-sm)', color: 'var(--text-primary)' }}>{m.name}</span>
                    {m.inspiration && <span title="Inspired" style={{ fontSize: 11 }}>⭐</span>}
                    {m.current_hp <= 0 && <span style={{ fontFamily: 'var(--font-heading)', fontSize: 9, color: 'var(--color-crimson-bright)', background: 'rgba(155,28,28,0.2)', padding: '1px 5px', borderRadius: 3 }}>Down</span>}
                  </div>
                  <div style={{ fontFamily: 'var(--font-heading)', fontSize: 9, color: 'var(--text-muted)', marginBottom: 3 }}>Lvl {m.level} {m.class_name}</div>
                  <div className="hp-bar-container">
                    <div className="hp-bar-fill" style={{ width: `${Math.max(0, Math.min(100, hpPct * 100))}%`, background: hpColor }} />
                  </div>
                </div>
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 'var(--text-md)', color: hpColor }}>{m.current_hp}{m.temp_hp > 0 ? `+${m.temp_hp}` : ''}</div>
                  <div style={{ fontFamily: 'var(--font-heading)', fontSize: 9, color: 'var(--text-muted)' }}>/ {m.max_hp} HP</div>
                  <div style={{ fontFamily: 'var(--font-heading)', fontSize: 9, color: 'var(--text-muted)' }}>AC {m.armor_class}</div>
                </div>
      ...
      {activeSection === 'notes' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', flex: 1 }}>
          ...
          <textarea
            ...
            rows={16}
            style={{ resize: 'vertical', fontSize: 'var(--text-sm)', lineHeight: 1.7, fontFamily: 'var(--font-body)', flex: 1 }}
          />
        </div>
      )}
    </div>
