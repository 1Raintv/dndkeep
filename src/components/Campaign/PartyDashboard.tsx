  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
      ...
      <div style={{
        ...
        <SummaryChip label="Party Members" value={characters.length} color="var(--text-primary)" />
        ...
        <SummaryChip
          ...
          color={characters.some(c => (c.active_conditions?.length ?? 0) > 0) ? '#f59e0b' : 'var(--text-muted)'}
        />
        <SummaryChip
          label="Concentrating"
          value={characters.filter(c => c.concentration_spell).length}
          color={characters.some(c => c.concentration_spell) ? '#a78bfa' : 'var(--text-muted)'}
        />
      </div>
...
function SummaryChip({ label, value, color }: { label: string; value: string | number; color: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      <div style={{ fontFamily: 'var(--font-body)', fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-muted)' }}>{label}</div>
      <div style={{ fontFamily: 'var(--font-heading)', fontWeight: 900, fontSize: 'var(--text-lg)', color, lineHeight: 1 }}>{value}</div>
    </div>
  );
}
...
function CharacterCard({ character: c }: { character: Character }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 'var(--radius-xl)',
      background: isDowned ? 'rgba(220,38,38,0.04)' : 'var(--bg-card)',
      overflow: 'hidden',
      transition: 'border-color var(--transition-fast)',
    }}>
      {/* HP accent bar */}
      <div style={{ height: 3, background: `linear-gradient(90deg, ${col}, ${col}40)`, width: `${hpPct * 100}%`, transition: 'width var(--transition-slow)' }} />

      <div style={{ padding: 'var(--space-3) var(--space-4)' }}>
        {/* Name row */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-2)' }}>
          <div>
            <div style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 'var(--text-md)', color: isDowned ? '#dc2626' : 'var(--text-primary)' }}>
              {c.name}
            </div>
          ...
          <div style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', fontWeight: 700, color: status.color, textAlign: 'right' }}>
            {status.label}
          </div>
        </div>

        {/* HP bar */}
        <div style={{ height: 8, background: 'var(--bg-sunken)', borderRadius: 999, overflow: 'hidden', marginBottom: 'var(--space-2)' }}>
          <div style={{ height: '100%', width: `${hpPct * 100}%`, background: col, borderRadius: 999, transition: 'width var(--transition-slow), background var(--transition-normal)', boxShadow: `0 0 6px ${col}` }} />
        </div>
        ...
        <div style={{ display: 'flex', gap: 'var(--space-3)', marginBottom: 'var(--space-2)' }}>
          ...
          <StatMini label="AC" value={c.armor_class} color="var(--color-gold-bright)" />
          <StatMini label="Speed" value={`${c.speed}ft`} color="var(--text-muted)" />
        </div>

        {/* Death saves (if downed) */}
        {isDowned && (
          <div style={{ display: 'flex', gap: 'var(--space-2)', alignItems: 'center', padding: 'var(--space-2) var(--space-3)', background: 'rgba(220,38,38,0.08)', borderRadius: 'var(--radius-md)', marginBottom: 'var(--space-2)' }}>
            <span style={{ fontFamily: 'var(--font-body)', fontSize: 10, fontWeight: 700, color: '#ff8a80' }}>DEATH SAVES</span>
        ...
        {c.concentration_spell && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 8px', background: 'rgba(167,139,250,0.1)', border: '1px solid rgba(167,139,250,0.3)', borderRadius: 999, marginBottom: 'var(--space-2)', width: 'fit-content' }}>
            <span style={{ fontSize: 12 }}>🔮</span>
            <span style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', color: '#a78bfa', fontWeight: 600 }}>
            ...
            {(c.active_conditions ?? []).map(cond => {
              ...
              return (
                <div key={cond} style={{
                  ...
                  color: mechanic?.color ?? 'var(--text-muted)',
                }}>
                  {mechanic?.icon} {cond}
                </div>
              );
            ...
            {[1,2,3,4,5,6,7,8,9].map(lvl => {
              ...
              return (
                <div key={lvl} style={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                  <span style={{ fontFamily: 'var(--font-body)', fontSize: 9, color: 'var(--text-muted)', marginRight: 1 }}>{lvl}</span>
                  {Array.from({ length: total }, (_, i) => (
                    <div key={i} style={{
                      width: 8, height: 8, borderRadius: '50%',
                      background: i < remaining ? '#a78bfa' : 'var(--bg-raised)',
                      border: `1px solid ${i < remaining ? '#a78bfa' : 'var(--border-dim)'}`,
                    }} />
                  ))}
                </div>
              );
        ...
        {/* Inspiration */}
        {c.inspiration && (
          <div style={{ marginTop: 'var(--space-2)', display: 'flex', alignItems: 'center', gap: 4 }}>
            <span style={{ fontSize: 12 }}>⭐</span>
            <span style={{ fontFamily: 'var(--font-body)', fontSize: 10, color: 'var(--color-gold-bright)', fontWeight: 700 }}>INSPIRED</span>
          </div>
        )}
      </div>
    </div>
...
function StatMini({ label, value, color }: { label: string; value: string | number; color: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
      <div style={{ fontFamily: 'var(--font-body)', fontSize: 8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-muted)' }}>{label}</div>
      <div style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 'var(--text-sm)', color, lineHeight: 1 }}>{value}</div>
    </div>
  );
}
