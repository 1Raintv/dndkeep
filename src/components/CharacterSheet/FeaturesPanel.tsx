...
export default function FeaturesPanel({ character, onUpdateNotes }: FeaturesPanelProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
      <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
        {(['class', 'notes'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            style={{
              padding: '4px var(--space-4)', borderRadius: 999,
              border: `1px solid ${tab === t ? 'var(--color-gold)' : 'var(--border-subtle)'}`,
              background: tab === t ? 'rgba(212,160,23,0.12)' : 'transparent',
              color: tab === t ? 'var(--color-gold-bright)' : 'var(--text-muted)',
              fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', fontWeight: 600, cursor: 'pointer',
            }}>
      ...
      {tab === 'class' && (
        <>
          <div style={{ padding: 'var(--space-3) var(--space-4)', background: 'rgba(212,160,23,0.06)', border: '1px solid rgba(212,160,23,0.2)', borderRadius: 'var(--radius-lg)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 'var(--text-sm)', color: 'var(--color-gold-bright)' }}>
            ...
            <button className="btn-ghost btn-sm" onClick={() => setShowAll(v => !v)} style={{ fontSize: 'var(--text-xs)' }}>
            ...
            <div key={level}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
                {grouped[String(level)].map(feature => {
                  ...
                  const borderColor = feature.isSubclassFeature ? 'rgba(91,63,168,0.3)' : feature.isASI ? 'rgba(52,211,153,0.2)' : 'var(--border-subtle)';
                  const bgColor = feature.isSubclassFeature ? 'rgba(91,63,168,0.05)' : 'var(--bg-sunken)';
                  const nameColor = feature.isSubclassFeature ? 'var(--color-arcane-bright)' : feature.isASI ? 'var(--hp-full)' : 'var(--text-primary)';

                  return (
                    <div key={key} style={{ border: `1px solid ${borderColor}`, borderRadius: 'var(--radius-md)', background: bgColor, overflow: 'hidden' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', padding: 'var(--space-2) var(--space-3)', cursor: 'pointer' }}
                        onClick={() => setExpandedFeature(isOpen ? null : key)}>
                        <span style={{ fontSize: 15, flexShrink: 0 }}>{getIcon(feature.name)}</span>
                        <div style={{ flex: 1 }}>
                          <span style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: 'var(--text-sm)', color: nameColor }}>
                            {feature.name}
                          </span>
                          {feature.isSubclassFeature && character.subclass && (
                            <span style={{ fontFamily: 'var(--font-body)', fontSize: 9, color: 'var(--color-arcane-bright)', background: 'rgba(91,63,168,0.15)', border: '1px solid rgba(91,63,168,0.3)', padding: '1px 5px', borderRadius: 999, marginLeft: 6 }}>
                              {character.subclass}
                            </span>
                          )}
                        </div>
                      ...
                      {isOpen && (
                        <div className="animate-fade-in" style={{ padding: 'var(--space-2) var(--space-3) var(--space-3)', borderTop: '1px solid var(--border-subtle)' }}>
...
Background abilities, tool proficiencies, languages...`}
            style={{ fontSize: 'var(--text-sm)', lineHeight: 1.7, resize: 'vertical' }}
          />
        </div>
      )}
    </div>
