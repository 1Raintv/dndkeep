...
export const CLASS_LEVEL_PROGRESSION: Record<string, LevelMilestone[]> = {
  ...
  Psion: [
    { level: 1,  features: ['Psionic Talent (INT-based spellcasting, 4 prepared spells)', 'Telepathic Sense (detect surface thoughts 30 ft, 1 min, INT mod uses/LR)'],
                 choices: [{ type: 'cantrips', label: 'Choose 2 cantrips (INT-based)' }, { type: 'spells', label: 'Prepare INT mod + 1 spells from the Psion list' }] },
    { level: 2,  features: ['Psionic Focus: concentration spells cost no material components', 'Psychic Defense: add INT mod to Wisdom saves'],
                 choices: [{ type: 'spells', label: 'Prepare INT mod + 2 spells' }] },
    { level: 3,  features: ['Mind Link: telepathic communication 120 ft (always active)'],
                 choices: [
                   { type: 'subclass', label: 'Choose Psionic Discipline (Metamorph / Psi Warper / Psykinetic / Telepath)' },
                   { type: 'spells', label: 'Prepare INT mod + 3 spells' },
                 ], newSpellLevel: 2 },
    { level: 4,  features: ['Psionic Augmentation: add INT mod to one concentration spell\'s save DC'],
                 choices: [ASI, { type: 'cantrips', label: 'Learn 1 additional cantrip' }, { type: 'spells', label: 'Prepare INT mod + 4 spells' }] },
    { level: 5,  features: ['Psionic Blast: when you cast a spell, one creature must make a WIS save or be frightened until end of next turn (INT mod uses/LR)'],
                 choices: [{ type: 'spells', label: 'Prepare INT mod + 5 spells' }], newSpellLevel: 3 },
    { level: 6,  features: ['Mental Fortitude: resistance to Psychic damage', 'Subclass feature'],
                 choices: [{ type: 'spells', label: 'Prepare INT mod + 6 spells' }], subclassFeature: true },
    { level: 7,  features: ['Psionic Ward: use Reaction to impose Disadvantage on one attack roll against you (INT mod uses/LR)'],
                 choices: [{ type: 'spells', label: 'Prepare INT mod + 7 spells' }], newSpellLevel: 4 },
    { level: 8,  features: [],
                 choices: [ASI, { type: 'spells', label: 'Prepare INT mod + 8 spells' }] },
    { level: 9,  features: ['Psionic Resilience: advantage on Concentration saves'],
                 choices: [{ type: 'spells', label: 'Prepare INT mod + 9 spells' }], newSpellLevel: 5 },
    { level: 10, features: ['Subclass feature', 'Astral Sight: cast Detect Thoughts without spell slot once per Long Rest'],
                 choices: [{ type: 'cantrips', label: 'Learn 1 additional cantrip' }, { type: 'spells', label: 'Prepare INT mod + 10 spells' }], subclassFeature: true },
    { level: 11, features: ['Psionic Surge: once per turn when you deal Psychic damage, add your INT modifier to the damage'],
                 choices: [{ type: 'spells', label: 'Prepare INT mod + 11 spells' }], newSpellLevel: 6 },
    { level: 12, features: [],
                 choices: [ASI, { type: 'spells', label: 'Prepare INT mod + 12 spells' }] },
    { level: 13, features: ['Mind Vault: store one prepared spell in your mind. It is always prepared and doesn\'t count against your limit'],
                 choices: [{ type: 'spells', label: 'Prepare INT mod + 13 spells' }], newSpellLevel: 7 },
    { level: 14, features: ['Subclass feature'],
                 choices: [{ type: 'spells', label: 'Prepare INT mod + 14 spells' }], subclassFeature: true },
    { level: 15, features: ['Psionic Dominance: cast Dominate Person once per Long Rest without a spell slot'],
                 choices: [{ type: 'spells', label: 'Prepare INT mod + 15 spells' }], newSpellLevel: 8 },
    { level: 16, features: [],
                 choices: [ASI, { type: 'spells', label: 'Prepare INT mod + 16 spells' }] },
    { level: 17, features: ['Absolute Focus: while concentrating, you cannot be Surprised and have advantage on Initiative'],
                 choices: [{ type: 'spells', label: 'Prepare INT mod + 17 spells' }], newSpellLevel: 9 },
    { level: 18, features: ['Psionic Mastery: cast two concentration spells simultaneously (the second doesn\'t require concentration)'],
                 choices: [
                   { type: 'other', label: 'Psionic Mastery: choose a 2nd-level or lower spell — it can be maintained alongside a concentration spell' },
                   { type: 'spells', label: 'Prepare INT mod + 18 spells' },
                 ], subclassFeature: true },
    { level: 19, features: [],
                 choices: [ASI, EPIC, { type: 'spells', label: 'Prepare INT mod + 19 spells' }] },
    { level: 20, features: ['Psionic Transcendence: your INT score increases by 4 (max 25). Immune to Psychic damage. Cast Telepathy and Foresight once per Long Rest without slot'] ,
                 choices: [{ type: 'spells', label: 'Prepare INT mod + 20 spells' }] },
  ],
