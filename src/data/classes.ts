  ...
  },
  {
    ...
    subclasses: [
      ...
      },
      {
        ...
        features: [
          { level: 3, name: 'Psionic Power', description: (c) => { const die = c.level >= 17 ? 'd12' : c.level >= 11 ? 'd10' : c.level >= 5 ? 'd8' : 'd6'; const pb = c.level >= 17 ? 6 : c.level >= 13 ? 5 : c.level >= 9 ? 4 : c.level >= 5 ? 3 : 2; return `You have ${pb * 2} Psionic Energy Dice (${die}) fueling Protective Field, Psionic Strike, and Telekinetic Movement. Regain 1 as a Bonus Action; all on a Short or Long Rest.`; }, descriptionLong: (c) => { const die = c.level >= 17 ? 'd12' : c.level >= 11 ? 'd10' : c.level >= 5 ? 'd8' : 'd6'; const pb = c.level >= 17 ? 6 : c.level >= 13 ? 5 : c.level >= 9 ? 4 : c.level >= 5 ? 3 : 2; return `You have a pool of Psionic Energy Dice equal to twice your proficiency bonus (currently ${pb * 2}), each a ${die} (d6 at levels 3\u20134, d8 at 5\u201310, d10 at 11\u201316, d12 at 17+). They fuel:\n\u2022 Protective Field \u2014 Reaction to reduce damage to a creature within 30 ft by 1 die + your Intelligence modifier.\n\u2022 Psionic Strike \u2014 once per turn after a hit within 30 ft, deal 1 die + Intelligence modifier Force damage.\n\u2022 Telekinetic Movement \u2014 move an object or creature 30 ft.\nYou regain one die as a Bonus Action (once per Short/Long Rest) and all dice on a Short or Long Rest.`; }, isChoice: false },
